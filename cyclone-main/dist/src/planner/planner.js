...
function planWind(windingParameters, verboseOutput = false) {
    const machine = new machine_1.WinderMachine(windingParameters.mandrelParameters.diameter, verboseOutput);
...
function planHoopLayer(machine, layerParameters) {
    ...
    const windAngle = 90 - (0, helpers_1.radToDeg)(Math.atan(layerParameters.mandrelParameters.diameter / layerParameters.towParameters.width));
    ...
    machine.move({
        ...
        ["mandrel" /* ECoordinateAxes.MANDREL */]: lockDegrees,
        ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: 0
    });
    // Tilt delivery head
    machine.move({
        ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: -windAngle
    });
    // Wind to the far end of the mandrel
    machine.move({
        ["carriage" /* ECoordinateAxes.CARRIAGE */]: layerParameters.mandrelParameters.windLength,
        ["mandrel" /* ECoordinateAxes.MANDREL */]: farMandrelPositionDegrees
    });
    // Do a small far lock
    machine.move({
        ["mandrel" /* ECoordinateAxes.MANDREL */]: farLockPositionDegrees,
        ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: 0
    });
    ...
    machine.move({
        ["carriage" /* ECoordinateAxes.CARRIAGE */]: 0,
        ["mandrel" /* ECoordinateAxes.MANDREL */]: nearMandrelPositionDegrees
    });
    // Do a small near lock
    machine.move({
        ["mandrel" /* ECoordinateAxes.MANDREL */]: nearLockPositionDegrees,
        ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: 0
    });
...
function planHelicalLayer(machine, layerParameters) {
    ...
    const towArcLength = layerParameters.towParameters.width / Math.cos((0, helpers_1.degToRad)(layerParameters.parameters.windAngle));
    ...
    const passRotationMM = layerParameters.mandrelParameters.windLength * Math.tan((0, helpers_1.degToRad)(layerParameters.parameters.windAngle));
    ...
    for (let patternIndex = 0; patternIndex < numberOfPatterns; patternIndex++) {
        ...
        for (let inPatternIndex = 0; inPatternIndex < patternNumber; inPatternIndex++) {
            ...
            for (let passParams of passParameters) {
                ...
                machine.move({
                    ["mandrel" /* ECoordinateAxes.MANDREL */]: mandrelPositionDegrees,
                    ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: 0
                });
                // Tilt delivery head to the start position for the pass
                machine.move({
                    ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: passParams.deliveryHeadSign * deliveryHeadPassStartAngle,
                });
                // Wind through the pass lead in, tilting the delivery head into final position
                mandrelPositionDegrees += leadInDegrees;
                machine.move({
                    ["carriage" /* ECoordinateAxes.CARRIAGE */]: passParams.leadInEndMM,
                    ["mandrel" /* ECoordinateAxes.MANDREL */]: mandrelPositionDegrees,
                    ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: passParams.deliveryHeadSign * deliveryHeadAngleDegrees,
                });
                // Wind to the end of the pass
                mandrelPositionDegrees += mainPassDegrees;
                machine.move({
                    ["carriage" /* ECoordinateAxes.CARRIAGE */]: passParams.fullPassEndMM,
                    ["mandrel" /* ECoordinateAxes.MANDREL */]: mandrelPositionDegrees
                });
                // Wind through the pass lead in, tilting the delivery head into final position
...
function planSkipLayer(machine, layerParameters) {
    ...
    machine.move({
        ["carriage" /* ECoordinateAxes.CARRIAGE */]: 0,
        ["mandrel" /* ECoordinateAxes.MANDREL */]: layerParameters.parameters.mandrelRotation,
        ["deliveryHead" /* ECoordinateAxes.DELIVERY_HEAD */]: 0
    });
    machine.setPosition({
        ["mandrel" /* ECoordinateAxes.MANDREL */]: 0,
